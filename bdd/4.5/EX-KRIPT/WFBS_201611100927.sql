...
create or replace PACKAGE BODY PKG_NIVEL AS
  ...
  BEGIN
    IF FN_TOKEN(p_IDUSUARIO,p_TOKEN)=1 then
        ...
        IF total = 0 THEN
        INSERT INTO NIVEL (NOTA,NOMBRE,DESCRIPCION,CREADO,MODIFICADO,DESACTIVADO) VALUES (p_NOTA,UPPER(p_NOMBRE),UPPER(p_DESCRIP),SYSDATE,SYSDATE,TO_DATE('01/01/1900','DD/MM/YYYY'));
        COMMIT;
        END IF;
        OPEN l_cursor FOR
      select * from 
...
IF FN_TOKEN(p_IDUSUARIO,p_TOKEN)=1 then
    OPEN l_cursor FOR
       SELECT *
        FROM  SELECCION
        ORDER BY encuesta_id,respuesta_id;
        APEX_JSON.open_object;
        APEX_JSON.write('seleccion', l_cursor);
        APEX_JSON.close_object; 
ELSE
...
IF FN_TOKEN(p_IDUSUARIO,p_TOKEN)=1 then
    IF total > 0 THEN
        OPEN l_cursor FOR
            SELECT *
            FROM  SELECCION
            WHERE encuesta_id=p_ENCUESTAID
...
IF FN_TOKEN(p_IDUSUARIO,p_TOKEN)=1 then
    ...
    SELECT (((p_NOTAAUTO*AUTO_PORC)/100) +  ((p_NOTAJEFE*JEFE_PORC)/100)) into nota FROM PERIODO WHERE ID = p_PERIODOID;
